...
|---|---|---|---|---|
| PointTaken (Sharath) | [active/pointtaken.md](active/pointtaken.md) | Custom, 50k credits/mo | `deriving-real-value` | Agency + design partner; 3 seats; heaviest self-serve-adjacent user; blocked by Richard context loss and credit cost |
| patchandbagel | [active/patchandbagel.md](active/patchandbagel.md) | Growth $99/mo | `deriving-real-value` | Healthiest true PLG account; lives in Cowork/agents/publishing, not image generation; renews Sep 13 |
...
| R for Rabbit | [active/r-for-rabbit.md](active/r-for-rabbit.md) | Retainer, 3 mo, $3,825 | `deriving-real-value` | 5 seats, canvas-heavy, team adoption real; no Stripe subscription so churn alerts won't fire |
...
| Sidharth Suresh | [active/sidharth-suresh.md](active/sidharth-suresh.md) | Pro $19/mo | `tried-not-getting-value` | Active 6 months, re-subscribed after card failure, but searches Spaces and almost never runs one |
| Imran Khan (Crafeed) | [active/imran-khan.md](active/imran-khan.md) | Growth $99/mo | `tried-not-getting-value` | Two identities, zero output on either; relationship owner unknown |
| Patricia Noble | [active/patricia-noble.md](active/patricia-noble.md) | Business $199/mo | `stopped-forgotten` | Bought 48 min after signup, tried for 3 sessions, silent since Jul 23 |
...
Eustress (109 searches / 0 executions → cancelled), hiaradigitals (22 / 0 → past_due), Patricia (13 / 0 → silent), and live: Sidharth (73 / 3, still paying). Sharath and Dawn — healthy — search a lot but *also* execute (327/60, 115/72). The ratio, not the search count, is the signal.
