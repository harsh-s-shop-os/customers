...
| Patricia Noble | [active/patricia-noble.md](active/patricia-noble.md) | Business $199/mo | `stopped-forgotten` | Habana Resortwear. 5 completed deck/catalogue generations confirmed in events; upgraded after exhausting the free plan; demo request 502'd four days before signup. Further output shown on recordings is unverified (see §4.12). Silent since Jul 23; renewed again Sep 20 |
...
### 4.1 Searching Spaces without running them precedes churn — `[Likely]`, n = 3 *(revised down 2026-09-21)*
Eustress (109 searches / 0 executions → cancelled), hiaradigitals (22 / 0 → past_due), and live: Sidharth (91 / 3, still paying). **Patricia was removed from this list 2026-09-21** — her 13 / 0 Spaces ratio is real but meaningless, because she was producing heavily in Creative Director and Cowork at the same time. A low Spaces ratio only signals anything when the account is not producing elsewhere, which is exactly what §4.10 says you cannot currently tell. Sharath and Dawn — healthy — search a lot but *also* execute (412/62 re-verified 2026-09-20, and 115/72). The ratio, not the search count, is the signal.
...
### 4.2 Fast or assisted conversion followed by zero self-produced output ends in churn — `[Likely]`, n = 3 *(revised down 2026-09-21)*
Eustress (Hemanth ran sample generations on a call → purchase → never generated alone), Imran/Crafeed (2 days then gone), hiaradigitals (bought day 4 → past_due). The purchase happened on promised or demonstrated value; the product did not carry it once the person was alone.

**Patricia was the fourth data point and has been withdrawn.** She submitted a Creative Director prompt on the free plan, exhausted the free allowance, and upgraded off the back of it — then produced 5 completed deck/catalogue versions (confirmed in events). Not a promise-buy. She still churned, so she belongs to §4.11 instead. *Corrected 2026-09-21: the earlier "credit wall mid-session on a paid plan" wording was wrong; `out_of_credits` has never fired on this account.*
...
Sidharth: six months, a card failure, and a `past_due` lapse with zero contact until 2026-09-16 — and he was *deliberately holding his credits* waiting for someone to tell him how to use the product. Patricia: **submitted a demo request four days before signing up and the endpoint 502'd**, so no lead was ever created; she then paid $199/mo self-serve, went silent in 11 days, and has renewed at least once with no human ever contacting her. She asked for a person first and the product ate the request. Eustress: contacted on the way in (Hemanth ran the samples), never after. Imran/Crafeed: relationship owner unknown. The self-serve base is unowned by default; the accounts that get a human are the ones that already had one at purchase (§4.4).

### 4.10 We were counting the wrong events, and some output genuinely isn't counted — `[Certain]`, n = 3 *(split and corrected 2026-09-21)*
Two separate problems were filed as one. Keep them apart, because the fixes are different.

**(a) Our method error — the bigger share of it.** Chat-led output *is* recorded, under `cowork_tool_executed` (`category: generation`, with a `tool_name` and `status`). We were only checking `space_generation_completed` and `creative_director_studio_generation_completed`. Patricia's file said "0 generations" while 5 completed `generateDeck` calls sat in her data the whole time. **Every "zero output" read in this workspace must be re-run against `cowork_tool_executed` before it is trusted** — that is a query fix, not an instrumentation fix, and it is actionable today.

**(b) A real gap — narrower than first claimed.** Image generation has no discoverable tool event: no `generateImage`-type `tool_name` appears in Patricia's data or in the sampled values for `cowork_tool_executed`. Nor does any event record a chat deletion, a failed in-app preview, or a download-format error. So image-side output and output-path bugs are genuinely invisible.

Sidharth's Creative Director output was missed until the 2026-09-20 recount (1 → 3). PointTaken's Creative Director figures were recorded as 37 / 85 / 2 when the truth is zero completions on any seat.

**Standing rule:** counters are sufficient to prove someone *did* produce; they are not sufficient to prove they didn't — but before blaming instrumentation, check whether we queried the right event. On Patricia we had not.

### 4.11 Producing the thing they came for does not by itself bring them back — `[Likely]`, n = 2 — **not settled**
Patricia finished a client catalogue (5 generated versions, last at 01:31 on Jul 13) and never produced again. *The "returned the next day to re-read it" detail is unverified — no events exist on Jul 14 (§4.12).* patchandbagel is the counter-case: also produces (41 published items) and *does* keep returning — but she publishes on a recurring cadence, where Patricia's job was a one-off deliverable.

The hypothesis: single-deliverable jobs complete and end the relationship, while recurring jobs create the return visit. If that holds, activation is the wrong metric for this base and "did they come back the following week without being prompted" is the right one. Needs a third account before it goes anywhere near a WBR.

---

### 4.12 Recording summaries are evidence, not facts — `[Likely]`, n = 1 — **not settled**
Patricia's recording summaries produced four claims that the event log could not support: a credit wall that never fired `out_of_credits`, a chat deletion at a timestamp where only a bare `api_call` is logged, and an entire Jul 13-evening / Jul 14 work block with **zero events of any kind, not even a `$pageview`**. Some of that is a real instrumentation gap (§4.10b). Some of it may be mis-dated or mis-attributed recordings. We do not currently know which.

Until that is resolved: cross-check recording-derived claims against the event log before writing them into a file as fact, and tag anything that fails the check. A recording showing activity in a window where *baseline* events are absent is a data-integrity question, not a customer insight.
...
| Patricia session recordings, Jul 2026 | Five PostHog replay summaries (all recordings that exist for her); saved at `interviews/patricia/2026-07-session-recordings.md` |
...
- 2026-09-21 (3) — **Cross-checked Patricia's recording-derived claims against the event log; three failed.** §4.10 split in two: (a) our own query error — chat-led output *is* recorded under `cowork_tool_executed`, we were checking the wrong events, and 5 completed `generateDeck` calls sat in her data all along; (b) a narrower real gap — image generation, chat deletion, preview failures and download-format errors have no event at all. New §4.12 added: recording summaries are evidence, not facts, after an entire claimed work block (Jul 13 evening + Jul 14) turned out to have zero events of any kind, not even `$pageview`. `active/patricia-noble.md` rewritten: credit-wall claim corrected to free-plan exhaustion (`out_of_credits` has never fired on that account), unverifiable blocks tagged, Read claims downgraded from `[Certain]` to `[Likely]`/`[Guessing]`.

- **2026-09-21** — Patricia's five July session recordings reviewed; the account file was substantially wrong and has been rewritten. She produced real finished work (campaign assets, Diwali shots, a 13-slide Spanish catalogue, a branded PDF catalogue) that no counter recorded; she upgraded at a credit wall mid-work rather than buying on promise; and her first-ever action — a demo request on 2026-07-08 — failed with a 502 nobody saw. Four output-path bugs logged. Consequences for this README: **§4.2 revised down to n = 3** (Patricia withdrawn as evidence), **§4.1 narrowed** (Patricia removed; a Spaces ratio only signals when the account isn't producing elsewhere), §4.9 sharpened, and two new patterns added — **§4.10, chat/Creative Director output is not instrumented, `[Certain]`, n = 3**, which puts a standing question mark over every zero-output read in this workspace; and **§4.11**, producing the thing they came for does not by itself bring them back (n = 2, not settled).
